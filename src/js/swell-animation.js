...
class SwellAnimation extends HTMLElement {
  ...
  updateFromAttributes() {
    ...
    // Convert direction to radians to match the arrow direction
    // Arrow shows swell direction, so lines should move perpendicular to that
    this.angleRad = ((direction + 90) * Math.PI) / 180;

    // Calculate speed based on canvas size and period
    const maxDimension = Math.max(
      this.canvas?.width || 200,
      this.canvas?.height || 100
    );
    
    // Adjust speed calculation for smaller card container
    this.speed = (maxDimension / period / 30) * speed * 0.5;

    // Set spacing between lines based on period (closer spacing for shorter periods)
  ...
  // Method to update swell parameters (called from main app)
